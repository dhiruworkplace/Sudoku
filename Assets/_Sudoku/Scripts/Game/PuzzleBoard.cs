...
namespace ArtboxGames
{
	public class PuzzleBoard : MonoBehaviour
	{
		...
		public PuzzleData					activePuzzleData;
		...
		public List<List<PuzzleBoardCell>>	activeCells;
		private List<NumberButton>			numberButtons;
		public int							selectedCellRow;
		public int							selectedCellCol;
		private bool						isNotesOn;

		[SerializeField] private List<Transform> NumberPositions;
		...
		public void SetNumber(int number)
		{
			...
			if (cellType == PuzzleData.CellType.Incorrect && number == cell.Number)
			{
				Erase();
				return;
			}
			...
			else
			{
				...
				if (wasPlaced)
				{
					...
					if (activePuzzleData.GetCellType(selectedCellRow, selectedCellCol) == PuzzleData.CellType.Incorrect)
					{
		...
		public void ToggleNotes()
		{
			...
			noteOnOffText.text		= isNotesOn ? "Note On" : "Note Off";
		...
		private void SetupNumberButtons()
		{
			...
			{           
                ...
                numberButton.GetComponent<LongClickBtn>().OnButtonClick = null;
				...
				numberButton.GetComponent<LongClickBtn>().OnButtonClick	+= OnNumberButtonClicked;
			}
		}
		...
		{
			if (gameManager.firstInputNum > 0)
			{
		...
		private void SetCellsSelected(int row, int col)
		{
			...
			if (gameManager.NumberFirstInput && gameManager.firstInputNum > 0)
			{
				SetAllCellsUnselected();
				SetNumber(gameManager.firstInputNum);				
			}
			else
			{
		...
		private void UpdateSelectedCells()
		{
			...
			if (gameManager.HighlightSameNumbers)
			{
		...
		/// </summary>
		public void SetAllCellsUnselected(bool isSelected = false)
		{
            if (activePuzzleData == null)
                return;
			...
			{
				PuzzleData.CellType cellType = activePuzzleData.GetCellType(selectedCellRow, selectedCellCol);
				SetCellColor(selectedCellRow, selectedCellCol, cellType);
			}
		}
		...
		/// </summary>
		public void SetCellsSelected(int row, int col, bool isSelected)
		{
            if (activePuzzleData == null)
                return;

            if (gameManager.HighlightConnectedCell)
			{
			...
			if (isSelected)
			{
				...
				SetCellColor(row, col, cellType);
			}
		}

		private void SetCellColor(int row, int col, PuzzleData.CellType cellType)
		{
			...
			{
				activeCells[row][col].SetHighlighted();
			}
			if (gameManager.HighlightSameNumbers && cellType != PuzzleData.CellType.Blank)
			{
				int number = activePuzzleData.GetNumber(selectedCellRow, selectedCellCol);
		...
		/// </summary>
		public void HighlightCells(int number, bool setHighlighted = true)
		{
			if (number == -1)
			{
				return;
			...
			else
			{
				...
				switch (cellType)
				{
					...
					case PuzzleData.CellType.Incorrect:
						gameManager.mistakesCount++;
						if (gameManager.MistakeLimit)
						{
                            gameManager.mistakesText1.text = gameManager.mistakesCount.ToString() + "/" + gameManager.mistakesLimit.ToString();
                            gameManager.mistakesText.text = gameManager.mistakesCount.ToString() + "/" + gameManager.mistakesLimit.ToString();
                        }
						else {
                            gameManager.mistakesText1.text = gameManager.mistakesCount.ToString();
                            gameManager.mistakesText.text = gameManager.mistakesCount.ToString();
                        }
						if (gameManager.mistakesCount >= gameManager.mistakesLimit && gameManager.MistakeLimit)
						{
							PopupManager.Instance.Show("gameover");
							AdsManager.Instance.ShowInterstitial();
						}
						cell.SetTextIncorrect();
						if (gameManager.Vibrate)
						{
							Vibration.VibratePop();
						}
						if (gameManager.isOnlinePlay)
						{
							ServerCode.Instance.SendMistakes(ServerCode.player.ConnectUserId, gameManager.mistakesCount);
						}
						break;
		...
		private void RemoveNotesForPlacedNumber(int row, int col, int number)
		{
			...
			if (GameManager.Instance.NoteDetection && activePuzzleData.GetCellType(row, col) != PuzzleData.CellType.Incorrect)
			{
		...
		private void CheckBoard()
		{
			...
			if (isSolved)
			{              
                if (gameManager.isOnlinePlay && ServerCode.Instance.winners.Count < 4) {
                    ServerCode.Instance.SendWinner(ServerCode.player.ConnectUserId);
                }
		...
		private void EnableNumberButtons()
		{
			...
			{
				//numberButtons[i].transform.position = NumberPositions[i].position;
